...
import type { CopyLintFinding } from "../copy-lint"
...
[최우선 원칙] 환각(사실 창작) 금지가 모든 "의무·필수" 규칙에 우선합니다. 초안에 근거 없는 사회적 증거·인증·시즌·연차가 있으면 채우지 말고 덜어내세요(정직 > 세일즈). 비제철(품종 제철과 현재월 불일치) 상품이면 "지금 제철/햇과일" 주장을 넣지 말고 예약·저장·시설·선별·보상 소구로 고치세요.
...
- 예외(3차 패스): 입력 origin이 참고 regions와 일치하면(예: origin "논산"이 딸기 regions에도 있음, "청송"이 사과 regions에도) 그건 정당한 산지 — 초안에서 그 지역명을 빼지 말고 유지하세요. 금지 대상은 origin과 "다른" 지역명뿐입니다.
...
- Brix 경계값(3차 패스): 입력 Brix가 그 품종 goodBrix와 같은 경계값이면 "달다"는 두되 "꿀맛·고당도 폭발" 같은 최상급은 "은은한 단맛/균형 잡힌 단맛"으로 절제해 고치세요.
- 품종 감각어(3차 패스): 초안이 품종 정체성(대극천=딱복·단단한, 설향=청량감)을 놓쳤으면 그 품종 감각어를 살려 다듬되, 감각어는 fact 풀 안에서만.
...
- 추상 슬로건 교정(3차 패스): highlightBox가 구체 감각 없이 시적 은유·추상만이면("향으로 먼저 익는 여름", "겨울을 담은 한 알") 색·식감·과즙·향 중 구체 감각 앵커를 넣어 고치세요("향부터 퍼지는 물오른 백도").
...
- 어미 다양화(규칙 31 > 59): 신뢰 섹션에 "~습니다"가 3문장 연속이면 1개를 "~답니다/~죠"로 풀어 고치세요.
- 비평 교정(3차 패스): 어색한 문어("산미가 여려"→"산미가 적어"), 억지 유도("~딱 좋은 이유가 있죠"→근거·확신 문장으로), "안심/정성" 남발(안심 2회 이내), 느낌표 과다(페이지 전체 4회 이내, 신뢰 섹션 0회)를 정리하세요.

[최상 기준 체크리스트 (v6.0 작업Q — 문장 교정을 넘어 "잘 팔리는 상세페이지"와 대조)]:
단순 오류 수정에서 멈추지 말고, 아래 4축을 최상 사례 기준으로 끌어올린 뒤 반환하세요. 단 모든 상향은 사실 정합 안에서만(환각 금지가 우선).
1. 훅 강도 — headline·heroKicker·highlightBox가 스크롤을 멈출 힘이 있는가. "누구나 할 말"(밋밋)이면 입력·fact·리서치에 이미 있는 사실(품종·감각·이미 입력된 수치·산지)만으로 날을 세우고, 없으면 각도·감각 표현으로 차별화하되 수치·산지를 새로 만들지 마세요(규칙 6 최우선). 이 상품이 아니어도 되는 문장이면 탈락.
2. 문단 리듬 — story·keyPoints body가 비슷한 길이 문장만 나열하지 않는가. 짧은 문장(끊어치기)과 긴 문장을 교차해 읽는 호흡을 만드세요.
3. 스캔성 — 모바일에서 훑어도 핵심이 잡히는가. keyPoints title·highlightBadges가 3초 스캔에 결론을 주는 라벨인가. 서술형이면 라벨형으로 압축하세요.
4. 어미 다양성 — 같은 어미(~습니다/~어요/~죠)가 3회 이상 연속되지 않는가(규칙 31·59). 연속되면 한둘을 변주해 단조로움을 깨세요.
...
export function buildRefineCopyMessages(
  ...
  // v6.0(작업Q): 초안 린터 위반이 있으면 "자동 수정 대상"으로 명시 주입.
  // 없으면(빈 배열·미전달) 블록 자체를 생략 → 위반 0인 초안은 현행과 동일한 심사(회귀 0).
  const lintBlock = buildLintFeedbackBlock(lintFindings)
  return [
    {
      role: "user",
      content: `원래 셀러 입력:
...
/**
 * v6.0(작업Q): 결정적 린터 위반 목록을 refine 프롬프트 블록으로 직렬화.
 * 위반이 없으면 빈 문자열(블록 생략) → 위반 0 초안은 기존 심사와 동일(회귀 0).
 * 필드·사유·문제 문구를 그대로 노출해 "어디의 무엇을 왜 고쳐야 하는지"를 명확히 지시한다.
 */
function buildLintFeedbackBlock(lintFindings?: CopyLintFinding[]): string {
  if (!lintFindings || lintFindings.length === 0) return ""
  const lines = lintFindings.map((f) => {
    const snippet = f.snippet?.trim()
    const where = snippet ? ` (문제 문구: "${snippet}")` : ""
    return `- ${f.field} [${f.code}]: ${f.reason}${where}`
  })
  return `
[자동 수정 대상 — 초안 린터 위반 목록 (v6.0 작업Q)]
아래는 초안에서 결정적 규칙 검사로 검출된 위반입니다. 해당 필드를 반드시 다시 다듬어 위반을 없애세요.
단 사실을 창작해 메우지 말고(환각 금지가 우선), 근거 없는 수치·산지·인증은 채우는 대신 덜어내 고치세요.
${lines.join("\n")}
`
}
