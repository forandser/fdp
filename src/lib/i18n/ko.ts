...
export const ko = {
  ...
  detail: {
    ...
    reviews: {
      title: "고객 후기 (선택)",
      hint: "실제 받은 후기만 넣어주세요 — 지어내면 안 돼요. 최대 5개까지 넣을 수 있어요.",
      textLabel: "후기 {n}",
      ...
      // v5.3(작업3): 후기 신뢰 메타 3종 — 전부 선택. 실제 받은 후기의 정보만 입력(지어내기 금지).
      ratingLabel: "별점 (선택)",
      ratingClear: "선택 안 함",
      authorLabel: "작성자 표기 (선택)",
      authorPh: "예) 김**",
      optionLabel: "구매 옵션 (선택)",
      optionPh: "예) 3kg",
      metaHint: "실제 받은 후기의 별점·작성자·옵션만 적어주세요. 비워두면 표시되지 않아요. 실명은 김** 처럼 가려서 적어주세요.",
      add: "+ 후기 추가",
      remove: "삭제",
      // v5.8(작업①): 후기 집계(선택) — 스토어에서 실제 집계된 숫자만. 지어내면 안 됨.
      statsTitle: "스토어 후기 집계 (선택)",
      statsHint: "스토어에서 실제 집계된 숫자만 입력하세요 — 지어내면 안 돼요. 비워두면 표시되지 않아요.",
      statsCountLabel: "누적 후기 수",
      statsCountPh: "예) 1240",
      statsFiveLabel: "5점 비율 (%)",
      statsFivePh: "예) 92",
      statsRepurchaseLabel: "재구매 지표 (자유 문구)",
      // v5.8(작업① fix): 예시에서 '1위' 등 검증 불가한 배타적 순위 주장 제거(표시광고법 리스크 유도 방지).
      statsRepurchasePh: "예) 재구매 많은 상품",
    },
    suggest: {
      title: "AI 추천 소구점",
    ...
    result: {
      ...
      // v5.8(작업①): 히어로 직하단 후기 집계 스트립 라벨·캡션. 입력된 값만 셀로 렌더.
      reviewStats: {
        countLabel: "누적 후기",
        countUnit: "건",
        fiveLabel: "5점 비율",
        repurchaseLabel: "재구매",
        caption: "판매자 스토어 집계 기준",
      },
      // v5.8(작업②): 벤토 데이터 그리드 — 개당/100g당 가격 셀·출하 기준 체크리스트·고르기 팁 라벨.
      bento: {
        priceLabel: "가격",
        // {won} = 콤마 구분 원 단위. 입력 가격+중량의 단순 산술 환산만(창작 없음).
        per100gUnit: "원 · 100g당",
        perEach: "개당 약 {won}원",
        // v5.8(작업② fix): 개당가는 품종 평균 중량으로 과수를 추정 → 입력값만이 아님을 명시(허위 방지).
        perEachNote: "품종 평균 {g}g 기준 추정",
        priceCaption: "※ 입력 중량 기준 단순 환산값이에요",
        shipmentTitle: "우리 농장 출하 기준",
        pickTipLabel: "고르기 팁",
        // v5.8(작업② fix): 중량 스펙 카드가 없을 때 입력 중량을 살리는 폴백 셀 라벨(정보 유실 방지).
        weightLabel: "중량",
      },
      deliveryTitle: "배송 안내",
