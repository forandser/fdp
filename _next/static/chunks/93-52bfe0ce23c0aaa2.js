"use strict";(self.webpackChunk_N_E=self.webpackChunk_N_E||[]).push([[93],{1310:(e,t,i)=>{i.d(t,{BB:()=>g,FK:()=>l,Ty:()=>b,cU:()=>d,dA:()=>x,le:()=>s,sC:()=>c,sx:()=>u});var r=i(3973),a=i(6149);async function n(){return await (0,r.Jt)(a.d.WORKS_DB)??{}}async function o(e){await (0,r.hZ)(a.d.WORKS_DB,e)}function s(){let e=Date.now().toString(36),t=Math.random().toString(36).slice(2,8);return`w_${e}_${t}`}async function x(e){let t=await n();t[e.id]={...e,updatedAt:e.updatedAt>0?e.updatedAt:Date.now()},await o(t)}async function l(){let e=Object.values(await n()).map(e=>({id:e.id,createdAt:e.createdAt,updatedAt:e.updatedAt,productName:e.productName,thumbDataUrl:e.thumbDataUrl}));return e.sort((e,t)=>t.updatedAt-e.updatedAt),e}async function d(e){return(await n())[e]??null}async function g(e){let t=await n();e in t&&(delete t[e],await o(t))}async function h(e){let t=await fetch(e);return await t.blob()}async function b(){let e=await n(),t=[];for(let i of Object.values(e)){let e=[];for(let t of i.imageBlobs)try{e.push(await function(e){return new Promise((t,i)=>{if("u"<typeof FileReader)return void i(Error("FileReader unavailable"));let r=new FileReader;r.onload=()=>t("string"==typeof r.result?r.result:""),r.onerror=()=>i(r.error??Error("read failed")),r.readAsDataURL(e)})}(t))}catch{}t.push({id:i.id,createdAt:i.createdAt,updatedAt:i.updatedAt,productName:i.productName,thumbDataUrl:i.thumbDataUrl,input:i.input,copy:i.copy,imagesBase64:e})}return{format:"fdp-backup",version:1,exportedAt:Date.now(),works:t}}async function u(e,t={}){if(!e||"object"!=typeof e||"fdp-backup"!==e.format)throw Error("INVALID_BACKUP");if(!Array.isArray(e.works))throw Error("INVALID_WORKS");let i=t.merge?await n():{},r=0,a=0;for(let t of e.works){if(!t||"string"!=typeof t.id){a++;continue}try{let e=[];for(let i of t.imagesBase64??[])"string"==typeof i&&i.startsWith("data:")&&e.push(await h(i));i[t.id]={id:t.id,createdAt:t.createdAt??Date.now(),updatedAt:t.updatedAt??Date.now(),productName:t.productName??"",thumbDataUrl:t.thumbDataUrl??null,input:t.input,copy:t.copy??null,imageBlobs:e},r++}catch{a++}}return await o(i),{imported:r,skipped:a}}async function c(e,t=240){try{let i=await createImageBitmap(e),r=Math.min(1,t/Math.max(i.width,i.height)),a=Math.max(1,Math.round(i.width*r)),n=Math.max(1,Math.round(i.height*r)),o=document.createElement("canvas");o.width=a,o.height=n;let s=o.getContext("2d");if(!s)return null;return s.drawImage(i,0,0,a,n),i.close(),o.toDataURL("image/jpeg",.7)}catch{return null}}},7448:(e,t,i)=>{i.d(t,{E:()=>P});var r=i(6129);let a="claude-opus-4-8";var n=i(4555);let o=[/\bignore\s+(all\s+)?(previous|prior|above)\b/i,/\bdisregard\s+(all\s+)?(previous|prior|above)\b/i,/\b(system|assistant)\s*:/i,/<\|im_(start|end)\|>/i,/<\|endoftext\|>/i,/\[INST\]/i,/<<SYS>>/i],s=/https?:\/\/[^\s)]+/gi,x=/<\/?[a-z][^>]*>/gi;function l(e){let t=String(e);for(let e of o)t=t.replace(e,"");return(t=(t=t.replace(s,"")).replace(x,"")).trim()}var d=i(9632);let g={freshness:["당일 수확","당일 발송","산지 직송","주문 확인 후 수확","새벽에 거둔","한 알 한 알 골라","햇~","수확 12시간 안에 출고","콜드체인 봉인 배송","수확 다음 날 도착","갓 따 보내드려요"],taste:["입맛을 깨우는","새콤달콤한","진한 향","한 입 가득","과즙이 터지는","단맛 위주","은은한 향","농축된 단맛","한 알의 단맛","꿀이 차오른 (Brix 수치 병기 시)"],texture:["아삭한","사각 소리가 나는","폭신한","탱글한","녹는 듯한","쫀쫀한","촉촉한","씹는 맛","톡 터지는","단단한"],farmer:["3대째 같은 밭에서","20년차 김 농부","손으로 한 알씩 골라","새벽 5시에 따요","저희 가족이 직접","오늘 아침에도 밭에 다녀왔어요","농부지인","산지에서 직접 보내드려요"],priceJustification:["손이 많이 들어요","박스 한 칸당 30분 걸려요","콜드체인 차량 비용이 박스당 1,800원 들어요","농약 대신 손으로 잡아서 키웠어요","한 알 평균 g 무게로 비교하면","송이 500g 이상만 골라요","올해 7월 우박 맞아 겉에 작은 흠집이 있어요","시즌 마감이라 가격을 30% 내렸어요","모양만 다르고 맛은 같아요"],honestFlaws:["모양은 들쑥날쑥, 맛은 그대로","겉에 작은 흠집이 있어요","크기가 균일하지 않아요","노지 특성상 색깔이 조금씩 달라요","표면의 분(가루)은 자연 발생이에요","신고는 11.4 Brix라 아주 단 카피는 못 쓰지만 아삭함이 차별점이에요"],pairing:["아침 식탁에 한 알","도시락에 한 송이","토스트 위에 올리면","요거트와 한 스푼","샐러드 토핑","선물 박스를 여는 순간","주말 가족 모임에","아이 간식\xb7이유식 보조"],scarcity:["이번 주 한정 출하","올해 첫 ~","여름 한정 햇과일","시즌 마감 임박","올해 마지막 출하분","농가 보유 한정 수량"],trust:["올해 산지 직배송 1,200건","3대째 같은 밭에서 30년째","주문 확인 후 새벽에 따서","GAP 인증 (인증번호 있을 때만)","지리적 표시 등록 산지","직접 측정한 농가 Brix 기록","단골 농가 지정 출하"]},h=`당신은 한국 산지직송 신선식품 셀러의 상세페이지 카피라이터입니다.
...
}`,b=`당신은 한국 신선식품(과일\xb7야채) 셀러를 위한 카피라이팅 조언자입니다.
...
13. JSON 한 개만, 다른 설명 없이.`,u=`당신은 한국 신선식품(과일\xb7야채) 셀러를 위한 검색\xb7SEO 키워드 조언자입니다.
...
12. JSON 한 개만, 다른 설명 없이.`,c={"claude-sonnet-4-6":{inputUsdPerMtok:3,outputUsdPerMtok:15},"claude-haiku-4-5":{inputUsdPerMtok:.8,outputUsdPerMtok:4},[a]:{inputUsdPerMtok:15,outputUsdPerMtok:75}};function m(e,t){return Math.max(0,Number.isFinite(t)?t:0)/1e6*c[e].inputUsdPerMtok*1380}function y(e,t){return Math.max(0,Number.isFinite(t)?t:0)/1e6*c[e].outputUsdPerMtok*1380}let M=new Set(["__proto__","constructor","prototype"]);function f(e,t){return e&&e.length>t?e.slice(0,t):e}function p(e){return"object"==typeof e&&null!==e&&!Array.isArray(e)}function v(e,t=""){return"string"==typeof e?e.trim():t}function k(e){return Array.isArray(e)?e.filter(e=>"string"==typeof e).map(e=>e.trim()).filter(Boolean):[]}function B(e){if(!e)throw Error("EMPTY_RESPONSE");let t=e.trim();t=t.replace(/^```(?:json)?\s*/i,"").replace(/```\s*$/i,"");try{return JSON.parse(t)}catch{throw Error("INVALID_JSON")}}var w=i(4849);function C(e){switch(e){case"ok":return w.t.diagnostic.success;case"invalid_key":return w.t.diagnostic.fail.invalid_key;case"geo_blocked":return w.t.diagnostic.fail.geo_blocked;case"rate_limited":return w.t.diagnostic.fail.rate_limited;case"network_error":return w.t.diagnostic.fail.network_error;case"unknown_error":return w.t.diagnostic.fail.unknown_error}}class A{async createClient(){let e=await (0,n.r)().getKey();if(!e)throw Error("API 키가 입력되지 않았습니다.");return new r.Ay({apiKey:e,dangerouslyAllowBrowser:!0})}async diagnose(){try{let e=await this.createClient(),t=await e.messages.create({model:this.modelId,max_tokens:8,messages:[{role:"user",content:"ok"}]}),i=Array.isArray(t.content)&&t.content.length>0;return{status:"ok",reachable:!0,modelAvailable:i,message:C("ok")}}catch(o){let e,t,i,r,a,n=(e=o?.status,t=(o?.error?.type??"").toLowerCase(),i=(o?.error?.message??"").toLowerCase(),r=(o?.message??"").toLowerCase(),a=`${t} ${i} ${r}`,401===e?"invalid_key":403===e||a.includes("unsupported_country_region_territory")||a.includes("country")||a.includes("region")?"geo_blocked":429===e||529===e?"rate_limited":a.includes("network")||a.includes("fetch")||a.includes("connection")||a.includes("aborted")?"network_error":"unknown_error");return{status:n,reachable:"network_error"!==n,modelAvailable:!1,message:C(n)}}}async generateCopy(e){var t;let i,r,a,n,o=await this.createClient(),s=(r=(i={...t=e,productType:l(t.productType),variety:t.variety?l(t.variety):void 0,origin:l(t.origin),weight:l(t.weight),storageHint:t.storageHint?l(t.storageHint):void 0,highlightKeywords:t.highlightKeywords.map(l).filter(e=>e.length>0)}).tone??"sincere",a=function(e){let t=(0,d.HH)(e.productType);if(!t)return"(fruit-facts 사전에 없는 상품 — 입력 단서만 사용해 카피하세요. 추측 금지.)";let i=d.Xp[t],r=[`fruit-facts 사전 매칭: "${t}"`,`- 카테고리: ${i.category}`,`- 사용 가능 감각어 (규칙 42): ${i.sensoryWords.join(", ")}`,`- "달다/꿀맛/고당도" 표현은 ${i.goodBrix} Brix 이상에서만 허용 (규칙 43)`,`- 보관 mode: ${i.storage.mode} — ${i.storage.note} (규칙 39)`];return null!=e.brix&&(e.brix>=i.goodBrix?r.push(`- 입력 Brix(${e.brix}) >= goodBrix(${i.goodBrix}) → "달다/고당도" 표현 허용`):r.push(`- 입력 Brix(${e.brix}) < goodBrix(${i.goodBrix}) → "달다/고당도/꿀맛" 어휘 일체 금지. 차별점을 식감\xb7향\xb7산지로 표현하세요.`)),!e.origin?.trim()&&i.regions.length>0&&r.push(`- 주요 산지(참고): ${i.regions.slice(0,3).join(", ")} — 입력에 산지 없으면 카피에 산지명 만들지 마세요.`),r.join("\n")}(i),n=function(){let e=["권장 표현 풀 (이 풀의 어휘를 우선 활용; 그 외 표현 가능하나 식약처 가이드 위배 X 확인 필수):"];for(let t of Object.keys(g)){let i=function(e){switch(e){case"freshness":return"신선도";case"taste":return"맛";case"texture":return"식감";case"farmer":return"농가";case"priceJustification":return"가격 정당화";case"honestFlaws":return"솔직한 결점 자백";case"pairing":return"페어링\xb7먹는 장면";case"scarcity":return"시즌\xb7희소성";case"trust":return"신뢰 인용"}}(t);e.push(`- ${i}: ${g[t].slice(0,6).join(", ")}`)}return e.join("\n")}(),[{role:"user",content:`입력 데이터 (JSON):
